<style>
    ...
    /* Internal note mode - yellow background */
    #editor-container.internal-note-mode {
        background-color: #fff9e6 !important;
    }

    #editor-container.internal-note-mode textarea {
        background-color: #fff9e6 !important;
    }
...
<script>
    ...
    function setStatus(status) {
        ...
        // Status configuration map for PQRS
        const statusConfig = {
        ...
        if (statusLabel) {
            ...
            statusLabel.className = 'text-dark fw-bold';
        }
    ...
    // Spinner: Mostrar al enviar comentario/respuesta o cambiar estado en PQRS
    document.getElementById('reply-form').addEventListener('submit', function(e) {
        ...
        const currentStatus = '<?= $pqrs->status ?>';
    ...
    // Spinner: Mostrar al asignar PQRS con Select2
    setTimeout(function() {
        ...
        if ($agentSelect.length) {
            $agentSelect.on('select2:select select2:clear', function(e) {
                ...
                if (e.type === 'select2:clear' || this.value === '') {
                    LoadingSpinner.show('Desasignando PQRS...');
                } else {
...
<!-- Lazy Loading for PQRS History (PERFORMANCE FIX) -->
